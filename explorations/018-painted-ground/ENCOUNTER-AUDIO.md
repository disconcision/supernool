...
Recovery synchronization: one melodic onset per reversed operation, plus one at the planar-to-spatial handoff, uses the same 78%/22% timeline as geometry. Changing recovery duration or the number of recorded operations changes note spacing. The scheduler looks up to 90 ms ahead; missed cues are dropped, and paused scrubbing produces no new recovery notes. Previously sounding notes can decay naturally. Geometry worker latency can still put the displayed mesh slightly behind the sequence/audio clock. The fixed sketch now demonstrates the six-move, eight-second recovery timing too.
