...
export async function setupSettings(scope:string,parent:HTMLElement,selector:string,waitForRocks=false){
 ...
 panel.append(group);
 const subset=(values:Values)=>Object.fromEntries(Object.entries(values).filter(([id])=>group.value==='shadow'?shadowIds.includes(id):group.value==='ground'?groundIds.includes(id):group.value==='char'?charIds.includes(id):true));
