...
/** One musical cue per reverse move, followed by the spatial return. */
export function recoveryCueTimes(count:number,duration:number){
 return [...Array.from({length:count},(_,i)=>i/Math.max(1,count)*recoveryReplayFraction*duration),recoveryReplayFraction*duration];
}
...
export function replayRecovery(records:RewriteRecord[],host:Term,progress:number,config:LayoutOptions,options:Options,hostSpread:number,flow=.8){
 ...
 if(records.length&&t<recoveryReplayFraction){
  ...
  const pose=u>=1?layout(step.after,flat):u<=0?layout(step.before,flat):transition(step.before,step.after,u,flat,step.kind,step.merge,surface,flow);
