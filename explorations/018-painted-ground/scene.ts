import {touchPrimary,initialInputMode,rememberInputMode} from './input-profile';
...
function frameTree(dt:number){
 ...
 const desired=Math.min(preferredZoom,Math.max(.25,fitZoom(camera,framePoints,innerWidth,innerHeight,.55*treeScale)));
...
$('inputMode').onchange=()=>{rememberInputMode(value('inputMode'));cancelPointer();releaseGrip(false);keys.clear();walkVelocity.setScalar(0);hoverId=undefined;spotlight=undefined;handFocus=false;lingerPoint=undefined;pin=undefined;clearMovement();ui(false);renderer.domElement.focus();};
($('inputMode') as HTMLSelectElement).value=initialInputMode();
