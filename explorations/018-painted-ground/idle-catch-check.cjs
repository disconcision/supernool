...
function fixture(seed=2){
 ...
 const game=new IdleCatch(rng),root=new T.Group(),stone=new T.Object3D();stone.position.set(1.8,.08,0);stone.userData.rockSeed=111;
 ...
 const tick=(allowed=true,dt=1/60)=>{game.update(dt,allowed,root,hands);if(game.active)game.poses.forEach((p,i)=>{hands[i].position.copy(p.position);hands[i].quaternion.copy(p.orientation);});};
...
for(let seed=1;seed<=16;seed++){
 const f=fixture(seed);f.until('scout');const start=f.stone.position.clone();
 let maxY=0;
 ...
 assert(!f.game.active,'Sessions end within bounded time');assert(maxY>1.8,'Throws have a visible airborne arc');
...
assert(catches>10);assert(misses>0);for(const phase of ['grip','notice','windup','flight','catch','miss','retrieve','return','place'])assert(phases.has(phase),phase);
for(const phase of ['scout','grip','lift','notice','spread','windup','throw','flight','catch','miss','retrieve','return','place']){
 const f=fixture();f.until(phase);const before=f.stone.position.clone();f.tick(false);
...
for(const cause of ['movement','contact','pin','rewrite','input']){
 ...
 assert.equal(actor.root.userData.idleCatch.phase,'rest',cause);assert(!actor.root.userData.idleCatch.held,cause);
}
console.log(`Idle catch: ${catches} catches, ${misses} misses; all phases, palm attachment, continuity, grounded settling, bounded sessions and interruption in every phase passed.`);
