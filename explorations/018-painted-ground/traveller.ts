...
export function createTraveller(scene:T.Scene,status:(message:string)=>void){
 ...
 return {ready:()=>requested==='checkpoint'||settled.has(requested),root,update,previewIdle:controller.previewIdle,stopIdlePreview:controller.stopIdlePreview,setCatchProps:controller.setCatchProps,setIdleCatch:controller.setIdleCatch,setTravelStyle:controller.setTravelStyle,setIdleWalkStyle:controller.setIdleWalkStyle,setIdleTerrain:controller.setIdleTerrain,setIdleMode:controller.setIdleMode,choose:select,setPalette(name:string){palette=name;variants.forEach(paint);},current:()=>current,locomotion:()=>running?'Run':active?.motion??'procedural',linkEnds(){
