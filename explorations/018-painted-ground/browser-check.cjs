...
await page.locator('#hudToggle').click();await page.screenshot({path:__dirname+'/assets/sigils-raised.png'});await page.keyboard.press('Escape');
...
for(let step=0;step<6;step++){
 ...
 if(step===2){assert.equal(await page.locator('[data-role="pull-cursor"]').count(),0);await page.screenshot({path:__dirname+'/assets/identity-forgiving-catch.png'});}
 if(step===0)await page.screenshot({path:__dirname+'/assets/catch-feedback.png'});
...
await page.locator('#hudToggle').click();await page.screenshot({path:__dirname+'/assets/goal-sigils.png'});await page.mouse.move(650,350);await page.mouse.wheel(0,-220);await page.waitForTimeout(900);await page.screenshot({path:__dirname+'/assets/zoomed-paint.png'});
await page.setViewportSize({width:980,height:896});await page.waitForTimeout(800);await page.screenshot({path:__dirname+'/assets/narrow-paint.png'});
await page.keyboard.press('Escape');await page.locator('#settingsButton').click();await page.locator('#settings summary').filter({hasText:'Tree & sigils'}).click();await page.locator('#sigils').selectOption('layered');await page.locator('#closeSettings').click();await page.locator('#hudToggle').click();await page.screenshot({path:__dirname+'/assets/sigils-layered.png'});
