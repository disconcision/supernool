...
prop.visible=floor.visible=(mode==='inspect'||mode==='upright');prop.position.copy(p).add(new T.Vector3(-1.6,.32,.6));floor.position.copy(p).setY(-.025);
if(mode!==previousMode){previousMode=mode;avatar.setIdleWalkStyle(mode==='upright'?'upright':'spider');avatar.setIdleMode((mode==='inspect'||mode==='upright')?'explore':'rest');}
avatar.update(now,dt,moving,camera,grip,brace,0,working,working?{velocity:new T.Vector3(),effort:.6}:undefined);
const ends=avatar.linkEnds();ribbon.update(now,dt,true,working,ends.from,ends.to,camera,.15);renderer.render(scene,camera);document.body.dataset.idleRoam=JSON.stringify(avatar.root.userData.idleRoam);document.body.dataset.fingerWalk=JSON.stringify(avatar.root.userData.fingerWalk);document.body.dataset.character=avatar.current();document.body.dataset.motion=avatar.locomotion();document.body.dataset.handPose=avatar.root.userData.travelHandPose;document.body.dataset.handWeight=String(avatar.root.userData.travelHandWeight);}requestAnimationFrame(frame);
