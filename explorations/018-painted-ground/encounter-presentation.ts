...
export function createEncounterPresentation(scene:T.Scene,origin:T.Vector3,treeScale:number){
 ...
 function update(dt:number,p:Pose|undefined,hostIds:string[]=[]){
  ...
  if(enabled&&p&&key!==lastKey){
   ...
   for(const id of ids){const c=makeGrowthCluster(id,settings);c.scale.setScalar(0);c.visible=false;foliage.add(c);clusters.set(id,c);}
  }
  for(const [id,c]of clusters){
   const point=p?.points.get(id),edge=p?.edges.find(e=>e.id===id);
   const emergence=edge?Math.min(1,edge.a.distanceTo(edge.b)/.9):0;
   const target=point?size*channels.leaves*emergence:0;
   // Follow worker poses continuously, including newly emerging/departing tips.
   const follow=1-Math.exp(-dt*12),grow=1-Math.exp(-dt*5);
   if(point){if(!c.visible)c.position.copy(point);else c.position.lerp(point,follow);}
   c.scale.setScalar(T.MathUtils.lerp(c.scale.x,target,grow));c.visible=c.scale.x>.001;
  }
