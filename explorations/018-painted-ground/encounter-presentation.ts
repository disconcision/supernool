...
export function createEncounterPresentation(scene:T.Scene,origin:T.Vector3,treeScale:number){
 ...
 function update(dt:number,p:Pose|undefined){
  ...
  if(foliage.visible)for(const [id,c]of clusters){const point=p!.points.get(id);c.visible=!!point;if(point){c.position.copy(point);c.scale.setScalar(size*channels.leaves);}}
  ...
  const status=document.getElementById('sequenceStatus');if(status)status.textContent=`${sequence.state} · ${sequence.age.toFixed(1)} s${sequence.paused?' · paused':''}${!sequence.automatic?' · manual preview':''}`;
  return channels;
 }
 function mount(host:HTMLElement,callback:(s:EncounterState)=>void){onJump=callback;const panel=document.createElement('details');panel.id='encounterSequence';
  ...
  range('sequenceAwakening','Awakening seconds',1,6,.25,3);range('sequenceRelease','Release seconds',.6,4,.1,1.8);range('sequenceRecovery','Regrowth seconds',3,15,.5,8);range('sequenceLeafSize','Living canopy size',.4,1.8,.05,1);range('sequenceLeafDensity','Living canopy density',.4,1.6,.1,1);range('sequenceFlash','Transition flash strength',0,2,.05,1);
  host.prepend(panel);
