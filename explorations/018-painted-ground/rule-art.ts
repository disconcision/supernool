...
export type TileStyle='formula'|'bar'|'seal'|'motion';
...
export function ruleTile(id:string,style:TileStyle='formula'){
 const rule=rules.find(r=>r.id===id)!;const form=tileForms[id];const tile=document.createElement('span');tile.className='rule-tile '+style;tile.style.setProperty('--ink',rule.color);tile.title=rule.name+' · '+(style==='bar'?rule.equation.replace(/→|↔/g,'='):rule.equation);tile.setAttribute('role','img');tile.setAttribute('aria-label',tile.title);
 ...
 if(style==='formula'){text(form.top,42,24,form.top.length>6?14:17);path('M 42 29 V 37 M 38 33 L 42 37 L 46 33');text(form.bottom,42,53,form.bottom.length>6?14:17);}
  ...
  svg.setAttribute('viewBox','0 0 84 54');
  const topSize=form.top.length>6?14:17,bottomSize=form.bottom.length>6?14:17;
  const context=document.createElement('canvas').getContext('2d');
  const width=(s:string,size:number)=>{if(!context)return s.length*size*.5;context.font=`${size}px Georgia`;return context.measureText(s).width;};
  const half=Math.min(35,Math.max(9,(width(form.top,topSize)+width(form.bottom,bottomSize))/4));
  text(form.top,42,21,topSize);path(`M ${42-half} 27 H ${42+half}`);text(form.bottom,42,45,bottomSize);
 }
  ...
  const node=(x:number,y:number,label:string)=>{
   // Opaque paper masks the connecting strokes; the tint alone lets them show through.
   const paper=document.createElementNS(ns,'circle');paper.setAttribute('cx',String(x));paper.setAttribute('cy',String(y));paper.setAttribute('r','10');paper.setAttribute('fill','#fffef9');svg.append(paper);
   const c=document.createElementNS(ns,'circle');c.setAttribute('cx',String(x));c.setAttribute('cy',String(y));c.setAttribute('r','9');c.setAttribute('fill','currentColor');c.setAttribute('opacity','.13');svg.append(c);text(label,x,y+4,12);};
