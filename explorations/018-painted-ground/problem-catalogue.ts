...
function mount(){
 ...
 const header=el('header'),intro=el('div');intro.append(el('div','ordinal','SUPERNOOL / PROBLEM LIBRARY'));const title=el('h2','','Twelve ways through the clearing');title.id='catalogueTitle';intro.append(title,el('p','','Every problem is available. Badges show a known solving route, not the only possible rule set. Counts are verified reference solutions, not claims of minimum difficulty.'));
 ...
 for(const p of problems){const ref=referenceSolutions[p.id][0],card=el('article','problem-card');cards.set(p.id,card);card.dataset.problem=p.id;
  ...
  const badges=el('div');badges.append(el('div','caption','Rules in this route'),ruleStrip(ref.rules));card.append(badges,el('p','',p.note));
  ...
  for(const alt of referenceSolutions[p.id].slice(1)){const a=el('div','alternative');a.append(el('p','',alt.steps.length+' moves · alternative route'),ruleStrip(alt.rules));details.append(a);}
 ...
 function sync(){const p=problems.find(p=>p.id===select.value)??problems[0],ref=referenceSolutions[p.id][0];launch.replaceChildren();const label=el('span','','TREE PROBLEM');label.append(el('strong','',p.title),el('span','',ref.steps.length+' reference moves · browse all 12'));launch.append(label,el('span','browse','↗'));launch.disabled=select.disabled;current.replaceChildren(el('span','','Reference route'),ruleStrip(ref.rules));for(const [id,c]of cards){c.setAttribute('aria-current',String(id===p.id));const b=c.querySelector('button')!;b.disabled=select.disabled;b.textContent=id===p.id?'Current problem':'Choose problem';}}
 ...
 for(const input of document.querySelectorAll<HTMLInputElement>('#ruleList input[data-rule]'))input.after(ruleTile(input.dataset.rule!));
}
