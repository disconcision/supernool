...
export class IdleRoam {
 ...
 get state(){return {phase:this.phase,age:this.age,thresholds:this.thresholds,positions:this.poses.map(p=>p.position.toArray())};}
 ...
 update(dt:number,allowed:boolean,walking:boolean,busy:boolean,root:T.Object3D,hands:T.Object3D[]){
  ...
  if(this.phase==='rest'){
   ...
   this.poses=hands.map(h=>({position:h.position.clone(),orientation:h.quaternion.clone()}));this.thresholds=[0,1].map(()=>2.1+this.random()*.8);this.speeds=[0,1].map(()=>.20+this.random()*.07);
  ...
  if(this.phase==='drift'){
   for(let i=0;i<2;i++){
    ...
    const p=this.poses[i],target=this.targets[i].clone();target.y+=Math.sin(this.age*.65+i)*.07;
    const delta=target.sub(p.position),step=delta.clone().setLength(Math.min(delta.length(),this.speeds[i]*dt));
    const next=p.position.clone().add(step);
    if(Math.hypot(next.x-root.position.x,next.z-root.position.z)>.78&&[.5,1,3].every(t=>this.safe(p.position.clone().addScaledVector(step,t))))p.position.copy(next);else this.target(i,root);
    if(delta.length()<.14&&this.age>3)this.target(i,root);
    p.orientation.slerp(root.quaternion.clone().multiply(new T.Quaternion().setFromAxisAngle(new T.Vector3(0,0,1),(i?-.18:.18)+Math.sin(this.age*.4+i)*.09)),1-Math.exp(-dt*1.4));
   }
