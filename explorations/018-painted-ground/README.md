...
[Rule tiles & the Noolbox](rule-workshop.html) compares arrow formula stamps, compact identity bars, seals and gesture sketches at several sizes, then flat and CSS-depth inventory trays. Page-only equipment toggles illustrate which recorded problem routes a collection supports. The playable Noolbox uses compact identity-bar tiles alongside the existing checkboxes. See [design notes](../../design/rule-interface-study.md) for data semantics and limits. Rebuild the verified static witnesses with `npm run problems:metadata`.

The inventory study keeps equipment and supported routes side by side, with a switch between pocket and hovering containers. Compact identity bars are its initial tile style; the scene now also uses compact bars, with available move directions retained in the accompanying text. Branch-mark node discs mask edges behind their glyphs.


## Larger crowns and retained systems roadmap

The library now includes 15 problems. **Branching echoes** starts at 23 nodes (2 reference moves), **The hollow crown** at 25 (4 moves), and **The full span** at 31 (2 moves). These explicitly compare size against solution length: compound matching/carrying, deep nested cancellation, and the present expansion cap. The 31-node limit is a provisional guard, not a certified performance maximum. All new reference routes are regenerated and replay-verified; the existing gesture/meaning/finite-intermediate-pose checks include them. The 31-node case was also solved through two actual pointer drags in the scene, with its intermediate 17-node tree inspected. Narrow screens zoom out substantially. The larger cancellation hint takes roughly 1.7 seconds in the local core check, an outstanding bounded-search cost.

[Rewriting systems roadmap](../../design/rewriting-systems-roadmap.md) retains the discussion of extra-variable rule schemas, spare-hand sampling, typed holes, conditions, binding, resources, strategies/reflection, goal policies, alternative sufficient kits, and casual wandering/energy ideas. These are future design possibilities; unary negation, extra-variable interaction and an energy economy are not implemented by this pass.

## Branch clipping repair and sigil habitats

Larger trees exceeded the old wood mesher's fixed x/z ±6 and y −1…11 sampling box. The canopy and sigils use the skeleton directly, so they continued beyond the abruptly truncated wood. The worker now fits a rectangular sampling domain around the full curved, thickened members, including root flare, smooth-union padding and outside samples for normals. Positions are transformed back to the scene's existing coordinate contract; anisotropic normal scaling is corrected. Resolution retains the chosen grid cell count, so stretching the sampled extent can reduce detail density. This fixes domain clipping, not every possible sub-voxel detail or remeshing shimmer. Buffer overflow is now rejected explicitly instead of silently returning an incomplete mesh.

`mesh-domain-check.ts` verifies large planar/spatial trees, intermediate factoring poses, surface near distal tips, clear grid boundaries, and normal transformation. The former capsule-pruning check remains in the suite. The 31-node scene and its 17-node factoring result were inspected and manipulated in-browser.

[Sigil habitats](sigil-workshop.html) is interface study 02: seven vector mounting concepts, number/variable alternatives, a scrubbed attachment sketch and the original 005 wood/rune reference. It is linked from the problem library and study catalogue. See [design/provenance notes](../../design/sigil-habitats.md). Current gameplay sigils remain unchanged.

## Hollow crown interaction notebook

[Interaction notebook](interaction-notebook.html) records route ambiguity, zero absorption contact semantics, strict pins versus atomic chunks, and the user's local named-subtree proposal. Its angle and folding diagrams are explanatory prototypes, not implemented geometry optimization or a definition system. Append future observations and decisions here or in linked design notes.

Route names are now selectable buttons. Choose one before gripping (or use 1–9); the choice is carried into that grip, including an explicitly shown hint route. While gripping, changing route requires returning below 10% in both visible and target progress. This provides a deterministic fallback for coincident directions without disabling rules. Spatial selection remains unchanged when no route was explicitly selected. Command-Z / Ctrl-Z uses existing tree history; Shift adds redo. During a grip it cancels the uncommitted preview first; during settling it asks the user to wait. These keys leave text inputs and scenery-editor shortcuts alone.

Audio is now off in the engine and UI on every page load. The sound-mode control is transient, excluded from saved-control restoration and default application, so old presets cannot re-enable it. Explicit unmute remains available; other sound parameters can still be saved. This is containment of the unresolved buzzing, not an audio fix.

Validation: typecheck and full Node suite pass, including the default-muted audio lifecycle assertion. In-app-browser Hollow crown playthrough completed factor → calculate → absorb → identity with all eleven rules enabled and no pin; Command-Z and Command-Shift-Z were verified after factoring. Factor/distribute target centers measured approximately 5.5 pixels apart in that camera view. Chrome was unavailable through the browser tool, so this pass used the in-app browser rather than the preferred Chrome surface. Body mode uses the same route intent path; a full body-mode playthrough was not repeated in this pass.

User correction: the explicit route-selection buttons and pre-grip selection introduced above were withdrawn. Earlier held-grip number shortcuts remain as before. Undo and default-muted audio remain. The HTML notebook is parked; further interaction design should be discussed in the task before implementation. Its implementation-status text is historical.

## Derived dragging comparison

**Encounter → Drag contacts** now switches between the authored checkpoint and derived surviving occurrences. `?drag=derived` opens the experiment explicitly; `?drag=authored` opens the comparison. Legal algebra rules and the mouse/body scoring and catch remain shared. New contacts derive from local occurrence paths, held-copy preference and resulting positions rather than the action-to-contact table. See [mechanisms and concrete witnesses](../../design/interaction-mechanisms.md) for policies, limitations and the ordered exploration list. This is a bounded experiment on the existing algebra, not the complete Nool pattern engine or an approved replacement.

For identity, pull the surviving A. For zero absorption, pull zero. Equal factors retain whichever whole copy is held. Numeric contributors may carry a computed result. Existing glyph-changing constructor IDs are reconciled to preserve operator glyphs; this explicit policy is not algebraically forced. A promoted root's retiring incoming edge no longer overwrites its surviving sigil position.

The controlled right-T factor/distribute routes remain only 2.726° apart in Hollow Crown; derivation does not solve that ambiguity. A four-drag mouse solution succeeded with every rule enabled and no pin or explicit route selection. Core checks cover all preset solution routes, whole-subtree identity retention, numeric meaning, unique IDs, and advertised endpoints. The frozen preview remains the prior checkpoint until deliberately refreshed.

## Continuous track choice and whole-pose glide

**Encounter → Route tracking** adds `Continuous + pose glide` and a `3 px` stickiness comparison alongside `Lock and catch`. It is independent of Drag contacts. Open `?mode=body&inhabited=1&drag=derived&tracking=glide` to force the new trial after settings restoration. Audio remains off; port 3101 is not rebuilt by this change.

Continuous selection reevaluates the nearest clamped start–target track throughout a grip. It has no 12% route lock or 82% catch. Release applies the selected rewrite when its destination is nearer than its start; moving back makes release cancel again. The tree eases from its actually displayed worker pose on a route switch, including interrupted glides, and release/cancel settles from that same visible state. Corresponding skeleton members are interpolated before remeshing; arbitrary mesh buffers are not blended. This is a 3D adaptation of the algebra demo's temporal glide, not multi-candidate spatial blending or a physical rope.

[Project knowledge index](../../design/knowledge/README.md) and [Dragology source/research note](../../design/knowledge/dragology.md) retain upstream source links, the Nool port history, exact policies, ambiguity witnesses and limitations. AGENTS.md points future dragging work there.

Validation: typecheck and full Node suite pass. New checks exercise late switching, endpoint release, overshoot, cancellation, stickiness and 395 competing preset route-pose pairs (including retarget/release during an unfinished glide). In-app-browser test: in the original clearing, the left `2x` reached a 100% regroup preview, then changed to a 100% additive-identity preview without returning to start; releasing committed Remove zero. This used mouse mode's G-held keyboard cursor. A separate actual pointer drag removed the right zero. No page errors were reported. Intermediate and endpoint tree views were inspected. Chrome was unavailable through the browser control surface; a complete body-mode solution and comprehensive mesh/performance audit remain unverified. Finite geometry checks do not certify every cross-route blend as aesthetically good.

Subsequent user report and audit: factoring's generic transition can retract the old incoming member of a surviving/promoted multiplication junction away from its outgoing branch attachment. This predates continuous tracking. The diagnostic and architectural clarification are in the Dragology knowledge note; this pass does not claim to repair that physical-motion defect. Preserve specialized physical motion even when drag contacts are derived.

## Fine control near overlapping routes

Encounter → Route tracking → **Continuous + fine control near overlaps** (`tracking=precision`) adds smoothly reduced sensitivity near competitive routes for body/keyboard pulls, plus capped modest stickiness. Sensitivity ranges from 100% to 30%; it integrates movement deltas so changes in gain cannot teleport the hand. Direct mouse dragging stays one-to-one. The prior continuous, sticky, and lock/catch comparisons remain available. Tree animation and the 200 ms glide are unchanged. See the Dragology knowledge note for exact parameters, limitations and validation. Typecheck/full core suite pass; the original left-2x regroup-to-identity correction was tested with the keyboard cursor in the in-app browser. Sustained body-mode feel still needs evaluation.

Precision follow-up: ordinary **Continuous + pose glide** is again the initial default (`tracking=glide` overrides saved controls). The user found fine control could make pulls feel stuck without sufficiently improving close-endpoint transitions. Precision remains an experiment; its arbitrary launch-route bias is now suppressed at the source and when the incumbent has zero progress. Full checks and a first-tap keyboard-cursor browser check pass. This does not resolve the larger visual difference when changing between nearby completed poses.
