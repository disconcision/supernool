...
(async()=>{
 ...
 await page.getByRole('button',{name:'Open Inspect',exact:true}).click();await page.locator('#idleCatch').selectOption('catch-only');await page.locator('#closeSettings').click();
 ...
 await page.locator('#idleCatch').selectOption('catch-only');
 ...
 await page.getByRole('button',{name:'Open Inspect',exact:true}).click();
 await page.locator('#idleCatch').selectOption('catch-only');await page.locator('#character').selectOption('olive-cape');
 ...
 assert.deepEqual(errors,[]);console.log('Chrome: both art figures, pickup / notice / flight / catch, widening separation, airborne and held walking exits, grounded settling and Finger-walk-only comparison passed. Screenshots:',output);
