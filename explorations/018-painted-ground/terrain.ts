...
export function makeClearing(scene:T.Scene){
 const catchProps:CatchProp[]=[],walkStones:T.Object3D[]=[],walkFungi:T.Object3D[]=[];
 ...
 for(let i=0;i<36;i++){const cluster=Math.floor(i/4),a=cluster*2.4,r=8.8+rand(cluster)*1.1,x=Math.cos(a)*r+(rand(i)-.5)*.7,z=Math.sin(a)*r+(rand(i+9)-.5)*.7,h=.12+rand(i+2)*.23;const mushroom=new T.Group();mushroom.name='Mushroom '+(i+1);mushroom.position.set(x,0,z);mushroom.userData.formation={id:'fungus-'+i,kind:'mushroom'};mushroom.userData.contact=new T.Vector3(0,h+.06,0);scene.add(mushroom);walkFungi.push(mushroom);touchPoints.push(new T.Vector3(x,h+.06,z));mushroom.attach(add(new T.CylinderGeometry(.025,.04,h,5),stem,new T.Vector3(x,h/2,z)));mushroom.attach(add(new T.SphereGeometry(.15,7,4,0,Math.PI*2,0,Math.PI/2),cap,new T.Vector3(x,h,z),new T.Vector3(1+rand(i)*.8,.4,1)));}
 const gates=[stone(-6.5,-7.5,1.35,1.65,200),stone(6.5,-7.5,1.35,1.65,210)],baseY=gates.map(g=>g.position.y);let opened=0;
 function syncEditableProps(){
  if(!rockAuthoring)return;const props=rockAuthoring.list();
  walkStones.splice(0,walkStones.length,...props.filter(o=>o.userData.formation.kind==='fragment'));
  walkFungi.splice(0,walkFungi.length,...props.filter(o=>o.userData.formation.kind==='mushroom'));
  catchProps.splice(0,catchProps.length,...walkStones.flatMap(o=>{
   const w=Math.max(o.scale.x,o.scale.z),h=o.scale.y,profile=catchStoneProfile(w,h);if(!profile)return [];
   o.updateWorldMatrix(true,false);const touch=o.userData.contact.clone().applyMatrix4(o.matrixWorld);
   return [{object:o,radius:Math.max(w,h)*.52,groundY:o.position.y,touch,...profile}];
  }));
 }
 document.addEventListener('scene-rocks-ready',syncEditableProps);
 document.addEventListener('grow-rock-touch-points',syncEditableProps);
 ...
 return {obstacles,touchPoints,catchProps,canFingerWalk(p:T.Vector3){
  ...
  return !walkFungi.some(o=>o.visible&&Math.hypot(p.x-o.position.x,p.z-o.position.z)<.4+Math.max(o.scale.x,o.scale.z)*.4);
