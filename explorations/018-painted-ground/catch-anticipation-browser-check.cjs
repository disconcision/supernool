...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});try{
 ...
 for(const character of ['blue-wrap','olive-cape']){
  ...
  await page.waitForFunction(()=>JSON.parse(document.querySelector('#world').dataset.idleCatch).phase==='windup',null,{timeout:90000});
  await page.evaluate(()=>{window.__anticipation=[];function frame(){const s=JSON.parse(document.querySelector('#world').dataset.idleCatch);window.__anticipation.push(s);if(['windup','throw'].includes(s.phase))requestAnimationFrame(frame);}frame();});
  ...
  assert(wind.length>15);assert(Math.min(...wind.map(s=>s.cue))<-.35&&Math.max(...wind.map(s=>s.cue))>.35,'Thrower feints in both directions');
