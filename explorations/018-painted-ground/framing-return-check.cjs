...
await page.getByRole('button',{name:'Open Encounter',exact:true}).click();await page.locator('#approach').click();await page.waitForFunction(()=>document.querySelector('#world').dataset.near==='true');await page.waitForTimeout(1600);await page.locator('#world canvas').focus();await page.keyboard.press('Space');await page.waitForFunction(()=>document.querySelector('#world').dataset.autoFraming==='true');await page.waitForFunction(()=>{const d=document.querySelector('#world').dataset;return Math.abs(+d.cameraZoom-+d.fitZoom)<.003},null,{timeout:30000});
...
assert.equal(await page.locator('#world').getAttribute('data-caught'),'true');await page.mouse.up();await page.waitForFunction(()=>document.querySelector('#world').dataset.busy==='false');
// Remain explicitly engaged for the slow fit and return comparisons.
await page.locator('#inputMode').selectOption('body');await page.locator('#world canvas').focus();await page.keyboard.press('Space');await page.waitForTimeout(6000);const out=await zoom();assert(out<start-.03,'Taller rewrite must zoom out');
