...
export function createInhabitation(renderer:T.WebGLRenderer,scene:T.Scene,camera:T.OrthographicCamera,treeMesh:T.Mesh,materials:(T.MeshStandardMaterial|T.MeshToonMaterial)[],transform:(p:T.Vector3)=>T.Vector3,scale:number,contacts:T.Vector3[],paint:{setLocalLight:(lights:{position:T.Vector3;intensity:number}[],tint:T.Color,reach:number,gain:number)=>void},sun:T.DirectionalLight,onDischarge?:(kind:ArcClass,power:number,pan:number)=>void){
 ...
 settings={departure,formation:envelope.arrival!==undefined?{progress:envelope.arrival,swirl:envelope.arrivalTurns??.65}:undefined,form:v('spiritForm'),opacity:n('spiritOpacity')*(envelope.arrival!==undefined?Math.sqrt(envelope.shadow):envelope.shadow)*dispersal,fringe:n('spiritFringe')*envelope.shadow*dispersal,texture:n('spiritTexture'),anger,size:n('spiritSize')*scale*envelope.cloudGrowth,colour,arcs:true,time:clock,enabled:true,drift:n('spiritDrift'),strike:candidates[0]??new T.Vector3(),strikePoints:candidates,visibility:v('spiritVisibility'),roil:n('spiritRoil')+(envelope.arrival!==undefined?2*Math.sin(Math.PI*envelope.arrival):0),seed,density:n('spiritDensity'),fray:n('spiritFray'),lightning:lastSettings,arcPreview:preview,arcGlow:n('spiritHalo'),cloudFlash:flash*n('spiritCloudFlash')};
