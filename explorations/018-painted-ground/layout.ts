...
export function transition(before:Term,after:Term,t:number,o:LayoutOptions,kind:string,merge:Record<string,string>={},surface?:Options,flow=0):Pose{
 ...
 // A surviving occurrence can become the root during reduction or recovery.
 // Its retired incoming member must not overwrite the surviving node position.
 for(const [id,p] of retracted)if(!b.points.has(id))points.set(id,p);
