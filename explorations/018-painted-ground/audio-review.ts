...
button.onclick=async()=>{
 ...
 try{
 const ctx=new OfflineAudioContext(2,44100*34,44100),bank=createEncounterAudio(ctx);bank.mix(.3,.65,.55,.75,0);
 for(let t=0;t<34;t+=2.6){const phase:AudioPhase=t<4?'dormant':t<7?'awakening':t<18?'active':t<20?'release':t<28?'recovery':'healthy';bank.environment(t,phase,(Math.sin(t*7)+1)/2);}
 ...
 const url=URL.createObjectURL(new Blob([bytes],{type:'audio/wav'}));(document.getElementById('player') as HTMLAudioElement).src=url;const download=document.getElementById('download') as HTMLAnchorElement;download.href=url;download.hidden=false;
 document.getElementById('metrics')!.textContent=JSON.stringify({seconds:34,peak,peakDb:20*Math.log10(peak),rms:Math.sqrt(squares/(rendered.length*2)),nonfinite,events:bank.inspect().events},null,2);status.textContent='Ready. Press Play below. This sketch does not change your scene settings.';
