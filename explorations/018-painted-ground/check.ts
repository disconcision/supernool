...
import {catchPull} from './interaction';import {readable} from './algebra';
const from={x:0,y:0},to={x:100,y:0};
assert.equal(catchPull(false,from,to,{x:81,y:0}).caught,false);
assert.equal(catchPull(false,from,to,{x:95,y:20}).caught,true);
assert.equal(catchPull(false,from,to,{x:200,y:0}).caught,true,'Fast overshoot must catch');
assert.equal(catchPull(true,from,to,{x:250,y:200}).caught,true,'Overshoot and sideways drift must retain the catch');
assert.equal(catchPull(true,from,to,{x:60,y:0}).caught,true);
assert.equal(catchPull(true,from,to,{x:54,y:0}).caught,false,'Deliberate retreat releases the catch');
for(const y of [0,40,100,500]){
 assert.equal(catchPull(false,from,to,{x:82,y}).caught,true,'Near-finished geometry accepts irrespective of sideways offset');
 assert.equal(catchPull(false,from,to,{x:50,y}).progress,.5,'Same displayed pose for the same progress, independent of offset');
 assert.equal(catchPull(false,from,to,{x:0,y}).caught,false,'Pure sideways movement cannot complete a route');
}
assert.equal(catchPull(false,from,to,{x:-50,y:0}).progress,0);
assert.deepEqual(catchPull(false,from,from,{x:100,y:100}),{caught:false,progress:0});
assert.equal(readable(initial()),'2x + 0 + y + 3x + 0');
assert.equal(readable(tree),'5x + y');
console.log('Catch acquisition, fast overshoot, lateral retention, retreat hysteresis and readable expressions passed.');

// Goal shape accepts independent commutation of its two operators.
assert.ok(solved(tree));
for(const n of walk(tree))for(const a of actions(n).filter(a=>a.key==='swap')){
 const swapped=replace(tree,n.id,a.result);assert.ok(solved(swapped));
 for(const m of walk(swapped))for(const b of actions(m).filter(a=>a.key==='swap'))assert.ok(solved(replace(swapped,m.id,b.result)));
}
assert.equal(solved(initial()),false);
assert.equal(solved({...tree,op:'*'} as typeof tree),false);
console.log('Structural goal accepts all four factor/sum orders and rejects unfinished or wrong trees.');
