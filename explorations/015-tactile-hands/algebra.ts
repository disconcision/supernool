...
export function solved(t:Term){return count(t)<=5&&[-2,0,1,3].every(x=>[-1,0,2].every(y=>evaluate(t,x,y)===5*x+y));}
// Hints search actual legal moves. They do not mutate the current term or auto-play a script.
export function hint(root:Term):{nodeId:string;action:Action}|undefined{
 ...
 while(at<queue.length&&at<8000){const q=queue[at++];if(solved(q.t)&&q.first)return q.first;if(q.depth>=9)continue;
  for(const n of walk(q.t))for(const a of actions(n)){const t=replace(q.t,n.id,a.result),k=key(t);if(seen.has(k))continue;seen.add(k);queue.push({t,first:q.first??{nodeId:n.id,action:a},depth:q.depth+1});}
 }
}
