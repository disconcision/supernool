...
assert.equal(catchPull(false,from,to,{x:89,y:0}).caught,false);
...
const misaligned=catchPull(false,from,to,{x:100,y:100});assert.equal(misaligned.caught,false);assert(misaligned.progress<.9,'Rejected paths must not look complete');
