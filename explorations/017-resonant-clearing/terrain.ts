...
export function makeClearing(scene:T.Scene){
 const obstacles:{x:number;z:number;r:number}[]=[],rand=(n:number)=>{const x=Math.sin(n*127.1+41.7)*43758.5;return x-Math.floor(x);};
 ...
 ground.setAttribute('color',new T.Float32BufferAttribute(gc,3));ground.computeVertexNormals();add(ground,new T.MeshStandardMaterial({vertexColors:true,roughness:1}),new T.Vector3());
 ...
 for(let i=0;i<36;i++){const cluster=Math.floor(i/4),a=cluster*2.4,r=8.8+rand(cluster)*1.1,x=Math.cos(a)*r+(rand(i)-.5)*.7,z=Math.sin(a)*r+(rand(i+9)-.5)*.7,h=.12+rand(i+2)*.23;add(new T.CylinderGeometry(.025,.04,h,5),stem,new T.Vector3(x,h/2,z));add(new T.SphereGeometry(.15,7,4,0,Math.PI*2,0,Math.PI/2),cap,new T.Vector3(x,h,z),new T.Vector3(1+rand(i)*.8,.4,1));}
 const routes=[[[0,0,14],[.3,0,8],[-.7,0,2],[-1,0,-3]],[[-1,0,-3],[-3,0,-5],[-6.5,0,-7.5],[-10,0,-12]],[[-1,0,-3],[2,0,-5],[6.5,0,-7.5],[10,0,-12]]];
 const exitMaterials:T.MeshBasicMaterial[]=[];
 routes.forEach((points,i)=>{const curve=new T.CatmullRomCurve3(points.map(p=>new T.Vector3(p[0],.035,p[2])));const material=new T.MeshBasicMaterial({color:i?'#879f98':'#d2ecda',transparent:true,opacity:i?.65:.9});add(new T.TubeGeometry(curve,60,.027,5,false),material,new T.Vector3());const halo=new T.MeshBasicMaterial({color:'#9ce4c7',transparent:true,opacity:i?.04:.13,depthWrite:false});add(new T.TubeGeometry(curve,60,.10,6,false),halo,new T.Vector3(0,-.007,0));if(i)exitMaterials.push(material);});
 const gates=[stone(-6.5,-7.5,1.35,1.65,200),stone(6.5,-7.5,1.35,1.65,210)],baseY=gates.map(g=>g.position.y);let opened=0;
 return {obstacles,update(open:boolean,dt:number){opened+=(Number(open)-opened)*(1-Math.exp(-dt*2));gates.forEach((g,i)=>g.position.y=baseY[i]-opened*2.3);exitMaterials.forEach(m=>{m.color.set(open?'#dfedbd':'#879f98');m.opacity=.65+opened*.3;});for(const o of obstacles)if(o.z===-7.5)o.r=1.35*.82*(1-opened);}};
}
