import * as T from 'three';import {addBackdrop} from './backdrop';import {makeClearing} from './terrain';import {sigil} from './sigils';import {setupHUD} from './hud';import {createSound} from './sound';import {directionalContact} from './navigation';import {rules,ruleId,ruleColor,Pin,allowsPin,advanceSpring,catchPull} from './interaction';import {createLehi} from './lehi';import {Gesture,gestures,scoreDrag} from './gestures';import {OrbitControls} from 'three/addons/controls/OrbitControls.js';
...
const renderer=new T.WebGLRenderer({antialias:true,alpha:true});renderer.setPixelRatio(Math.min(devicePixelRatio,1.5));renderer.setClearColor('#bac7c1');renderer.shadowMap.enabled=true;renderer.shadowMap.type=T.PCFShadowMap;$('world').append(renderer.domElement);
...
const backdrop=addBackdrop(scene,$('world'),renderer);
const clearing=makeClearing(scene),obstacles=clearing.obstacles;
...
function updateRunes(pose:Pose){runes.children.forEach(o=>(o as T.Sprite).material.dispose());runes.clear();for(const [id,n] of pose.nodes){const p=pose.points.get(id);if(!p)continue;const s=new T.Sprite(sigil(n.kind==='op'?(n.op==='*'?'×':'+'):n.kind==='num'?String(n.value):n.name,n.kind==='op',id===selected,value('sigils')==='paper'));s.position.copy(p).add(new T.Vector3(0,.22,.05));s.scale.setScalar((n.kind==='op'?.98:.78)*(id===selected?1.12:1));s.userData.nodeId=id;s.renderOrder=10;runes.add(s);}runes.visible=near;}
...
type Grip={id:string;options:Gesture[];chosen?:Gesture;intent?:Gesture;progress:number;target:number;velocity:number;ready:boolean;cursorReady:boolean;caught:boolean;cursor:ScreenPoint;from:ScreenPoint;keyboard:boolean;body:boolean;avatarStart:T.Vector3};
...
function updateGrip(at:ScreenPoint){if(!grip)return;const h=grip;
 ...
 const chosen=h.caught&&h.chosen?h.chosen:h.intent??(h.chosen&&h.target>.12?h.chosen:best.g);const a=anchors(chosen),result=catchPull(h.caught,a.from,a.to,at),was=h.caught;
 ...
 if(h.chosen)status(h.caught?'Caught · release to complete. Retreat past halfway to undo the catch.':h.chosen.instruction+'. '+(h.body?'Move to pull; step back to relax.':'Follow the path; return to reverse.'));
...
function releaseGrip(allow=true){const h=grip;if(!h)return;const g=h.chosen,commit=allow&&h.ready;if(h.body)clearMovement();grip=undefined;controls.enabled=true;renderer.domElement.style.cursor='grab';epoch++;lastKey='';
 ...
 if(g&&(commit||h.progress>.005)){lastGesture=g;const before=tree;if(commit){history.push({tree,steps});future=[];tree=g.after;steps++;hoverId=undefined;selected=find(tree,g.gripId)?g.gripId:g.action.result.id;spotlight=undefined;}
...
function setHandFocus(on:boolean){if(grip||animation)return;if(on&&(!near||spread>.12||!loaded)){status('Let the tree settle, then press Space to reach in.');return;}
...
function tick(now:number){requestAnimationFrame(tick);const dt=Math.min(.05,(now-last)/1000);last=now;if(document.hidden)return;
 ...
 near=handFocus||!!grip||!!animation||Math.hypot(avatar.position.x-treeOrigin.x,avatar.position.z-treeOrigin.z)<(nearOld?7.5:5.5);
 ...
 clearing.update(solved(tree)&&!grip&&!animation,dt);$('world').dataset.caught=String(grip?.caught??false);
 ...
 $('pullStatus').textContent=bodyMode()&&animation?'SETTLING · The tree is catching up; hand control resumes shortly':grip?.body?`PULL · Keep Space held · Spring ${Math.round(grip.progress*100)}% · pull ${Math.round(grip.target*100)}%${grip.ready?' · CAUGHT — release':''}`:bodyMode()?(handFocus?'HAND · Arrows choose a sigil · Hold Space, then move to pull · Esc leaves':'WALK · Arrows move the traveller · Space reaches into the tree'):'Hold a rune and pull · Shift-click to pin';
}
$('backdrop').onchange=()=>backdrop.set(value('backdrop'));$('previewSound').onclick=()=>{sound.unlock();sound.catch();};$('soundMode').onchange=()=>{sound.unlock();sound.setMode(value('soundMode'));};$('volume').oninput=()=>sound.setVolume(+value('volume'));setupHUD();ui();requestAnimationFrame(tick);
