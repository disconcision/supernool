...
export function createLehi(scene:T.Scene){
 ...
 function update(now:number,dt:number,moving:boolean,camera:T.Camera,grip?:T.Vector3,brace?:T.Vector3,active=1,engaged=true){
  const time=now/1000;
  legs.forEach((leg,i)=>leg.rotation.x=moving?Math.sin(time*10+i*Math.PI)*.45:0);
  arms.forEach((arm,i)=>{const contact=i===active?grip:brace;arm.rotation.x=contact?-.8:moving?-Math.sin(time*10+i*Math.PI)*.3:0;arm.rotation.z=contact?(i?-.22:.22):0;});
  ...
  hands.forEach((h,i)=>{
   ...
   h.joints.forEach((j,k)=>j.rotation.x=T.MathUtils.lerp(j.rotation.x,contact&&(engaged||i!==active)?-.85-k*.1:.08,1-Math.exp(-dt*16)));
