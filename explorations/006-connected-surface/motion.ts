...
export type Segment={a:V;b:V;r:number};
export type Settings={rule:string;mapping:string;complexity:string;anchor:string;path:string;t:number};
...
export function pose(s:Settings){
 ...
 const edge=(a:V,b:V,r=.23)=>{segments.push({a,b,r});};
 ...
 if(s.rule==='identity'){
  ...
  edge(P,A);if(t>0){edge(P,B,.23*Math.min(1,t*5));joint('zero','0',B);}
 }
 const outer=s.rule==='regroup'&&s.mapping==='exchange'?lerp(P,Q,t):P;
 edge(v(0,0),outer,.36);if(s.rule!=='identity'||t>0)joint('p','+',P);
 function operand(id:string,pos:V,nested:boolean){joint(id,nested?'+':id.toUpperCase(),pos);if(nested){const X=v(pos.x-.7,pos.y+1.3,pos.z),Y=v(pos.x+.7,pos.y+1.3,pos.z);edge(pos,X,.17);edge(pos,Y,.17);joint(id+'x','x',X);joint(id+'y','y',Y);if(s.complexity==='uneven'){const Z=v(Y.x+.4,Y.y+1.1,Y.z),W=v(Y.x-.4,Y.y+1.1,Y.z);edge(Y,Z,.13);edge(Y,W,.13);joint(id+'z','z',Z);joint(id+'w','w',W);}}}
