...
function sync(){if(shadowStudy){const patch=['S7','S8','S9'].includes(val('style')),url=patch?new URL('./round-02/foliage.png',import.meta.url).href:conceptURL;$('concept').style.backgroundImage=`url(${url})`;$('concept').style.backgroundSize=patch?'300% 200%':'200% 200%';($('conceptLink') as HTMLAnchorElement).href=url;}if(shadowStudy)$('patchControls').hidden=!['S7','S8','S9'].includes(val('style'));if(shadowStudy)for(const id of ['visibility','roil'])($(id) as HTMLInputElement).disabled=['S1','S2','S3'].includes(val('style'));if(growthStudy)for(const id of ['normalSharing','shadeDepth','brushScale'])($(id) as HTMLInputElement).disabled=val('finish')!=='painted';if(growthStudy)($('backing') as HTMLInputElement).disabled=['G3','F5'].includes(val('style'));for(const name of ['seed','size','density','spread','pose','wind','light',...(growthStudy?['offshoots','backing','normalSharing','shadeDepth','brushScale']:[]),...(shadowStudy?['opacity','fringe','texture','anger','branchGlow','roil','barkFlow','effectTime','patchDensity','fray']:[])])$(name+'Value').textContent=num(name).toFixed(name==='seed'||name==='light'?0:2);$('method').textContent=labels[val('style')]+(growthStudy&&val('finish')==='painted'?' Painterly volume adds shared canopy normals, grouped colour and irregular brush marks.':'');for(const id of ['density','normals'])($(id) as HTMLInputElement).disabled=!growthStudy&&val('style')!=='F5';const pos:Record<string,string>={S7:'50% 100%',S8:'100% 100%',S9:'50% 0%',S4:'0% 0%',S5:'0% 0%',S6:'0% 0%',S1:'0% 0%',S2:'0% 0%',S3:'0% 0%',G1:'50% 100%',G2:'50% 0%',G3:'100% 100%',F1:'0% 0%',F2:'50% 0%',F3:'100% 0%',F5:'50% 100%',F6:'100% 100%'};$('concept').style.backgroundPosition=pos[val('style')];wind.strength.value=num('wind');woodMat.wireframe=checked('wire');}
...
$('foliage').onchange=()=>{foliage.visible=checked('foliage');if(currentPose)applyPose(currentPose);};$('sigils').onchange=()=>{if(currentPose)applyPose(currentPose);};if(shadowStudy)for(const id of ['opacity','fringe','texture','anger','branchGlow','roil','barkFlow','effectTime','patchDensity','fray'])$(id).oninput=sync;$('wind').oninput=sync;$('light').oninput=sync;
