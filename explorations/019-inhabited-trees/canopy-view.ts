import {setupSettings} from '../018-painted-ground/settings';
...
import {ArcPreview} from './storm-lightning';
...
const arcControls=['smallRate','mediumRate','largeRate','smallSize','mediumSize','largeSize','rockShare','arcDuration','burstiness','stormScale','restrikes'];let arcPreview:ArcPreview|undefined;
...
if(shadowStudy){const form=new URLSearchParams(location.search).get('form');if(form&&Array.from(($('style') as HTMLSelectElement).options).some(o=>o.value===form))($('style') as HTMLSelectElement).value=form;}
...
function sync(){if(shadowStudy){for(const input of document.querySelectorAll<HTMLInputElement|HTMLButtonElement>('#lightningControls input,#lightningControls button'))input.disabled=['S1','S2','S3'].includes(val('style'));}if(shadowStudy){const patch=['S7','S8','S9'].includes(val('style')),url=patch?new URL('./round-02/foliage.png',import.meta.url).href:conceptURL;$('concept').style.backgroundImage=`url(${url})`;$('concept').style.backgroundSize=patch?'300% 200%':'200% 200%';($('conceptLink') as HTMLAnchorElement).href=url;}if(shadowStudy)$('patchControls').hidden=!['S7','S8','S9'].includes(val('style'));if(shadowStudy)for(const id of ['visibility','roil'])($(id) as HTMLInputElement).disabled=['S1','S2','S3'].includes(val('style'));if(growthStudy)for(const id of ['normalSharing','shadeDepth','brushScale'])($(id) as HTMLInputElement).disabled=val('finish')!=='painted';if(growthStudy)($('backing') as HTMLInputElement).disabled=['G3','F5'].includes(val('style'));for(const name of ['seed','size','density','spread','pose','wind','light',...(growthStudy?['offshoots','backing','normalSharing','shadeDepth','brushScale']:[]),...(shadowStudy?['opacity','fringe','texture','anger','branchGlow','roil','barkFlow','effectTime','patchDensity','fray',...arcControls]:[])])$(name+'Value').textContent=num(name).toFixed(name==='seed'||name==='light'?0:name==='largeRate'?3:2);$('method').textContent=labels[val('style')]+(growthStudy&&val('finish')==='painted'?' Painterly volume adds shared canopy normals, grouped colour and irregular brush marks.':'');for(const id of ['density','normals'])($(id) as HTMLInputElement).disabled=!growthStudy&&val('style')!=='F5';const pos:Record<string,string>={S7:'50% 100%',S8:'100% 100%',S9:'50% 0%',S4:'0% 0%',S5:'0% 0%',S6:'0% 0%',S1:'0% 0%',S2:'0% 0%',S3:'0% 0%',G1:'50% 100%',G2:'50% 0%',G3:'100% 100%',F1:'0% 0%',F2:'50% 0%',F3:'100% 0%',F5:'50% 100%',F6:'100% 100%'};$('concept').style.backgroundPosition=pos[val('style')];wind.strength.value=num('wind');woodMat.wireframe=checked('wire');}
...
$('foliage').onchange=()=>{foliage.visible=checked('foliage');if(currentPose)applyPose(currentPose);};$('sigils').onchange=()=>{if(currentPose)applyPose(currentPose);};if(shadowStudy)for(const id of ['opacity','fringe','texture','anger','branchGlow','roil','barkFlow','effectTime','patchDensity','fray',...arcControls])$(id).oninput=sync;$('wind').oninput=sync;$('light').oninput=sync;
if(shadowStudy){for(const [id,kind] of [['previewSmall','small'],['previewMedium','medium'],['previewBranch','branch'],['previewRock','rock']] as const)$(id).onclick=()=>{arcPreview=kind;($('animateEffects') as HTMLInputElement).checked=false;sync();};$('resumeArcs').onclick=()=>{arcPreview=undefined;($('animateEffects') as HTMLInputElement).checked=true;};$('animateEffects').onchange=()=>{arcPreview=undefined;};$('sampleFlash').onclick=()=>{arcPreview='small';($('animateEffects') as HTMLInputElement).checked=false;($('effectTime') as HTMLInputElement).value=String(hash('delay0')*.16+.015);sync();};$('effectTime').oninput=()=>{arcPreview=undefined;($('animateEffects') as HTMLInputElement).checked=false;sync();};}
...
function frame(now:number){requestAnimationFrame(frame);const elapsed=(now-previous)/1000,dt=Math.min(.05,elapsed);previous=now;clock+=dt;wind.time.value=clock;orbit.update();avatar.update(now,dt,false,camera);
 ...
 if(shadow){const colour=s==='blue'?'#507fcf':s==='amber'?'#c79548':'#9260d9';const legacy=['S1','S2','S3'].includes(val('style')),effectTime=checked('animateEffects')?clock:num('effectTime');woodMat.emissive.set(colour);woodMat.emissiveIntensity=legacy?num('branchGlow')*.65:0;if(barkEnergy){barkEnergy.barkTint.value.set(colour);barkEnergy.barkClock.value=effectTime;barkEnergy.barkPower.value=legacy?0:num('branchGlow');barkEnergy.barkFlow.value=num('barkFlow');}for(const sigil of sigils.children){const m=sigil.userData.front;if(m){m.emissive.copy(m.color);m.emissiveIntensity=.65;}}shadow.render(scene,camera,{opacity:num('opacity'),fringe:num('fringe'),texture:num('texture'),anger:num('anger'),size:num('size'),colour,form:val('style'),arcs:checked('arcs'),time:effectTime,visibility:val('visibility'),roil:num('roil'),seed:num('seed'),density:num('patchDensity'),fray:num('fray'),lightning:{smallRate:num('smallRate'),mediumRate:num('mediumRate'),largeRate:num('largeRate'),smallSize:num('smallSize'),mediumSize:num('mediumSize'),largeSize:num('largeSize'),rockShare:num('rockShare'),duration:num('arcDuration'),burstiness:num('burstiness'),stormScale:num('stormScale'),restrikes:num('restrikes'),timingSeed:num('seed')},arcPreview,strikePoints:rocks.map(r=>r.position.clone().add(new T.Vector3(0,r.scale.y*.75,0))),strike:strikePoint,drift:num('wind'),enabled:checked('foliage')});}else renderer.render(scene,camera);frames++;total+=elapsed;if(total>.6){const info={fps:Math.round(frames/total),triangles:renderer.info.render.triangles,calls:renderer.info.render.calls,geometries:renderer.info.memory.geometries,style:val('style'),clusters:terminalIds.length,canopyTriangles};$('stats').textContent=`${info.fps} fps · ${shadowStudy?(shadow?.inspect().patches?'shadow '+shadow.inspect().patches!.cards+' patches':'cloud '+(shadow?.inspect().volumes??0)+' volumes'):'canopy '+info.canopyTriangles.toLocaleString()+' tris'} · scene ${info.calls} draws`;document.body.dataset.stats=JSON.stringify(info);frames=0;total=0;}}
setupSettings(shadowStudy?'shadow-019':growthStudy?'growth-019':'canopy-019',document.querySelector('aside')??document.querySelector('section')??document.body,'input,select').then(()=>{sync();requestPose();requestAnimationFrame(frame);});
