...
function sync(){if(growthStudy)($('backing') as HTMLInputElement).disabled=['G3','F5'].includes(val('style'));for(const name of ['seed','size','density','spread','pose','wind','light',...(growthStudy?['offshoots','backing']:[]),...(shadowStudy?['opacity','fringe','texture','anger','branchGlow']:[])])$(name+'Value').textContent=num(name).toFixed(name==='seed'||name==='light'?0:2);$('method').textContent=labels[val('style')];for(const id of ['density','normals'])($(id) as HTMLInputElement).disabled=!growthStudy&&val('style')!=='F5';const pos:Record<string,string>={S1:'0% 0%',S2:'0% 0%',S3:'0% 0%',G1:'50% 100%',G2:'50% 0%',G3:'100% 100%',F1:'0% 0%',F2:'50% 0%',F3:'100% 0%',F5:'50% 100%',F6:'100% 100%'};$('concept').style.backgroundPosition=pos[val('style')];wind.strength.value=num('wind');woodMat.wireframe=checked('wire');}
