...
export type PatchOptions={form:string;seed:number;size:number;density:number;fray:number;time:number;drift:number;roil:number};
...
export function makeShadowPatches(){
 ...
 const u={clock:{value:0},roil:{value:.8},fray:{value:.55},depth:{value:null as T.Texture|null},resolution:{value:new T.Vector2()}};
 ...
 varying vec2 vUv;varying vec3 patchLocal;uniform sampler2D brush,depth;uniform vec2 resolution;uniform float clock,fray,kind;
 ...
 build(p,o);const size=renderer.getDrawingBufferSize(new T.Vector2());if(target.width!==size.x||target.height!==size.y)target.setSize(size.x,size.y);u.resolution.value.copy(size);u.depth.value=depth;u.clock.value=o.time*o.drift;u.roil.value=o.roil;u.fray.value=o.fray;
 const projected:ProjectedLobe[]=[];const matrix=new T.Matrix4();
 for(const [id,g]of groups){const e=p.edges.find(e=>e.id===id)!;const scale=Math.min(1,e.a.distanceTo(e.b)/.75)*(p.nodes.get(id)?.kind==='op'?.64:1)*o.size;g.visible=scale>.001;g.position.copy(e.b);g.scale.setScalar(scale);g.quaternion.identity().slerp(new T.Quaternion().setFromUnitVectors(new T.Vector3(0,1,0),e.b.clone().sub(e.a).normalize()),.25);g.updateMatrixWorld(true);
 for(const obj of g.children){if(!(obj instanceof T.InstancedMesh)||!g.visible)continue;for(let i=0;i<obj.count;i+=12){obj.getMatrixAt(i,matrix);const c=new T.Vector3().setFromMatrixPosition(matrix).applyMatrix4(g.matrixWorld),p=c.clone().project(camera);const r=.62*scale;const dx=c.clone().add(new T.Vector3(r,0,0)).project(camera).sub(p),dy=c.clone().add(new T.Vector3(0,r,0)).project(camera).sub(p),dz=c.clone().add(new T.Vector3(0,0,r)).project(camera).sub(p);projected.push({x:p.x*.5+.5,y:p.y*.5+.5,rx:Math.hypot(dx.x,dy.x,dz.x)*.5,ry:Math.hypot(dx.y,dy.y,dz.y)*.5});}}
 }
