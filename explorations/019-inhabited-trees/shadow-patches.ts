import {CloudFormation,formationPoint,formationGLSL} from './cloud-formation';
...
export type PatchOptions={form:string;seed:number;size:number;density:number;fray:number;time:number;drift:number;roil:number;departure?:CloudDeparture;formation?:CloudFormation};
...
export function makeShadowPatches(){
 ...
 const u={arrival:{value:1},swirl:{value:.65},dissolve:{value:0},clock:{value:0},roil:{value:.8},fray:{value:.55},depth:{value:null as T.Texture|null},resolution:{value:new T.Vector2()}};
 ...
 const births=new Float32Array(obj.count);for(let i=0;i<obj.count;i++)births[i]=hash(id+':'+i+':formation');obj.geometry.setAttribute('birth',new T.InstancedBufferAttribute(births,1));
 ...
 uniform float clock,roil,arrival,swirl;attribute float birth;varying vec2 vUv;varying vec3 patchLocal;varying float formed;
 ${formationGLSL}
 void main(){vUv=uv;vec3 q=(instanceMatrix*vec4(position,1.)).xyz;
 formed=formationPhase(arrival,birth);
 if(arrival<1.){vec3 centre=instanceMatrix[3].xyz,local=q-centre;float rest=1.-formed;
 // Broad intersecting membranes condense, rather than little isolated particles.
 float angle=(birth-.5)*swirl*8.*rest,c=cos(angle),s=sin(angle);
 local.xy=mat2(c,-s,s,c)*local.xy;local*=1.+.35*rest;
 q=formationPoint(centre,birth,formed,swirl)+local;}
 patchLocal=q;
 ...
 varying vec2 vUv;varying vec3 patchLocal;varying float formed;uniform sampler2D brush,depth;uniform vec2 resolution;uniform float clock,fray,kind,dissolve;
 ...
 uv+=warp*(.065*fray+.055*(1.-formed));float paint=texture2D(brush,uv).a;
 ...
 // Grow a continuous density field inside each overlapping patch, not a hard card edge.
 float vapour=(1.-smoothstep(.1,1.,length((vUv-.5)*2.)))*smoothstep(.12,.62,grain);
 coverage=mix(vapour,coverage,formed*formed)*sqrt(formed);
 ...
 build(p,o);const size=renderer.getDrawingBufferSize(new T.Vector2());if(target.width!==size.x||target.height!==size.y)target.setSize(size.x,size.y);u.arrival.value=o.formation?.progress??1;u.swirl.value=o.formation?.swirl??.65;u.resolution.value.copy(size);u.depth.value=depth;u.clock.value=o.time*o.drift;u.roil.value=o.roil;u.fray.value=o.fray;u.dissolve.value=o.departure?.dissolve??0;
 ...
 for(const obj of g.children){if(!(obj instanceof T.InstancedMesh)||!g.visible)continue;for(let i=0;i<obj.count;i+=12){obj.getMatrixAt(i,matrix);let c=new T.Vector3().setFromMatrixPosition(matrix);if(o.formation)c=formationPoint(c,(obj.geometry.getAttribute('birth') as T.InstancedBufferAttribute).getX(i),o.formation);c.applyMatrix4(g.matrixWorld);const p=c.clone().project(camera);const r=.62*scale*(o.departure?.scale??1);const dx=c.clone().add(new T.Vector3(r,0,0)).project(camera).sub(p),dy=c.clone().add(new T.Vector3(0,r,0)).project(camera).sub(p),dz=c.clone().add(new T.Vector3(0,0,r)).project(camera).sub(p);projected.push({x:p.x*.5+.5,y:p.y*.5+.5,rx:Math.hypot(dx.x,dy.x,dz.x)*.5,ry:Math.hypot(dx.y,dy.y,dz.y)*.5});}}
 }
