...
export function validateScene(value:any):asserts value is SceneVersion{
 ...
 if(!value.controls||typeof value.controls!=='object'||Array.isArray(value.controls)||Object.keys(value.controls).length>400)throw new Error('Invalid scene settings');
 for(const [key,v] of Object.entries(value.controls))if(!/^[a-zA-Z][\w-]{0,79}$/.test(key)||!(typeof v==='boolean'||typeof v==='string'&&v.length<=(key==='groundFootPlacements'?32000:200)))throw new Error('Invalid setting');
