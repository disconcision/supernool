export type RockPlacement={id:string;kind:'basalt-group'|'bedrock';position:[number,number,number];scale:[number,number,number];yaw:number};
...
export function validateScene(value:any):asserts value is SceneVersion{
 ...
 for(const r of value.rocks){
  if(!r||typeof r.id!=='string'||!/^rock-\d+$/.test(r.id)||ids.has(r.id)||!['basalt-group','bedrock'].includes(r.kind))throw new Error('Invalid rock identity');ids.add(r.id);
