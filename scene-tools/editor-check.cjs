...
(async()=>{const sceneId='check-'+Date.now(),folder=path.resolve('scenes',sceneId),b=await chromium.launch({channel:'chrome',headless:true});try{
 ...
 const url='http://127.0.0.1:3100/explorations/018-painted-ground/?mode=body&rockLayout=enclosed&editor=1&scene='+sceneId;
 ...
 const invalid=await page.request.post('http://127.0.0.1:3100/__scene-editor/'+sceneId,{data:{...first,rocks:[{...first.rocks[0],scale:[0,1,1]}]}});assert.equal(invalid.status(),400);
 const foreign=await page.request.post('http://127.0.0.1:3100/__scene-editor/'+sceneId,{headers:{Origin:'https://unrelated.example'},data:first});assert.equal(foreign.status(),403);

 // Formation-only saves from the previous editor still load; new props use authored defaults.
 const legacyResponse=await page.request.post('http://127.0.0.1:3100/__scene-editor/'+sceneId,{data:{...first,title:'Legacy formation-only scene',rocks:first.rocks.filter(r=>r.id.startsWith('rock-'))}});assert.equal(legacyResponse.status(),201);const legacy=await legacyResponse.json();
 await page.request.put('http://127.0.0.1:3100/__scene-editor/'+sceneId+'/default',{data:{versionId:legacy.versionId}});
