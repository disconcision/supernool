...
(async()=>{const sceneId='check-'+Date.now(),folder=path.resolve('scenes',sceneId),b=await chromium.launch({channel:'chrome',headless:true});try{
 ...
 assert.deepEqual(errors,[]);console.log('Scene editor: real gizmo drag, numeric edit, undo/redo, play/edit isolation, file saves, settings+layout reload, retained versions, explicit default promotion, input/origin validation passed.');
