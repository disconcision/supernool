...
export function sceneStorage():Plugin{return {name:'scene-storage',configureServer(server){
 const root=join(server.config.root,'scenes');server.watcher.add(root);
 server.middlewares.use('/__scene-editor',async(req,res,next)=>{
  res.setHeader('Content-Type','application/json');res.setHeader('Cache-Control','no-store');
  const reply=(status:number,body:any)=>{res.statusCode=status;res.end(JSON.stringify(body));};
  try{
   const parts=(req.url??'').split('?')[0].split('/').filter(Boolean),sceneId=parts[0];
