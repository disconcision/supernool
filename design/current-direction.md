...
### Continuous route choice and pose glide trial

The user approved trying Dragology's nearest-track selection plus whole-display easing. Encounter → Route tracking compares continuous/no catch (initial trial), continuous with 3 px stickiness, and the existing lock/catch checkpoint. Both pointer and body input remain. This is separate from authored/derived contacts. Read `knowledge/dragology.md` before altering this mechanism: fixed grab-start previews, actual displayed snapshots on switches, live targets during easing, and discrete release decisions must remain distinct. The broader project knowledge index is `knowledge/README.md`. Multi-candidate spatial blending and new disambiguation gestures are not implemented by this pass.

User clarification during the continuity trial: physical tree deformation should retain tailored motions for different operations. Derived algebra/contact correspondence must not be conflated with generic physical interpolation. A factoring audit found a surviving junction's old incoming member retracting away from its attachment; this predates the new track-switch glide. It remains an open physical-motion bug, recorded with a reproducer in `knowledge/dragology.md`. Existing finite-pose checks are not connectivity guarantees.

The user approved an ambiguity-precision trial after finding continuous route choice still too sensitive to single arrow presses. `tracking=precision` adds lower per-delta body/keyboard input gain near competing tracks and modest capped stickiness, preserving the existing glide and physical motions. It is a comparison awaiting feel feedback; previous tracking modes remain available. Details and tradeoffs are in `knowledge/dragology.md`.

Latest precision feedback: the user reports harder starts and continued abruptness near close endpoints. Ordinary continuous pose glide is restored as the initial default; precision remains an unresolved comparison, with its zero-progress incumbent bias corrected. Preserve this feedback and the remaining pose-switch problem; see `knowledge/dragology.md`.

2026-09-09: user says current dragging feels better and can stay for now, but remains undecided; revisit when evaluating interaction scope rather than treating it as finalized. A concrete original-tree audit confirms the held `((2x+0)+y)` plus has four derived targets versus two authored. The difference is handle eligibility and junction correspondence, not globally arbitrary rewrites or the latest scoring change. See `knowledge/dragology.md` → Target scope audit for exact results, Nool/Dragology comparisons and undecided scope alternatives. No target pruning is authorized by this discussion yet.
